module WB_reg(
           ...
           output wire [31: 0] load_data_ext,
...
wire [31: 0] load_data;
wire [31: 0] douta;
reg [1: 0] ld_bytes_select;// the last 2 bit of data memory address
wire [3:0] w_en;
initial
begin
    IR_WB = 32'b0;
...
assign w_en = addr[15:8]==8'hff ? 4'b0000 : mem_write_en;
DataMem DataMem (
            .clk (clk),
            .we (w_en << addr[1: 0]),//if write w_en=1 and discriminate the type of data
            .addr (addr[14: 2]),//only useful for data of word 
...
assign load_data_new = (addr[15:8]==8'hff)? io_din : douta;

assign load_data = stall_ff ? load_data_old :
                             clear_ff ? 32'b0 : 
                              load_data_new;
data_ext data_ext(//process the data one byte, half word, one word
            .data_in(load_data),
            .ld_bytes_select(ld_bytes_select),
            .reg_write_WB(reg_write_WB),
            .data_out(load_data_ext)
        );
