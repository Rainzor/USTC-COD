...
reg [            31:0] cache_mem    [SET_SIZE:0][WAY_CNT:0][LINE_SIZE:0]; // SET_SIZE个line，每个line有LINE_SIZE个word
reg [TAG_ADDR_LEN-1:0] cache_tags   [SET_SIZE:0][WAY_CNT:0];            // SET_SIZE个TAG
reg                    valid        [SET_SIZE:0][WAY_CNT:0];            // SET_SIZE个valid(有效位)
reg                    dirty        [SET_SIZE:0][WAY_CNT:0];            // SET_SIZE个dirty(脏位)
...
reg [2:0] cache_stat;// cache 状态机的状态定义
localparam IDLE = 2'b00, // IDLE代表就绪，SWAP_OUT代表正在换出，SWAP_IN代表正在换入，SWAP_IN_OK代表换入后进行一周期的写入cache操作。
            SWAP_OUT = 2'b01, 
            SWAP_IN = 2'b10,
            SWAP_IN_OK = 2'b11;
...
reg  [31:0] mem_wr_line [LINE_SIZE:0];
wire [31:0] mem_rd_line [LINE_SIZE:0];
...
reg [31:0] record [SET_SIZE:0][WAY_CNT:0];

always @ (*) begin
    if(cache_hit) begin // hit
        for ( j = 0; i < WAY_CNT; i = i + 1)
        begin
...
always @ (posedge clk or posedge rst) begin     // ?? cache ???
    if(rst) begin
        ...
        SWAP_IN_OK: begin           // 上一个周期换入成功，这周期将主存读出的line写入cache，并更新tag，置高valid，置低dirty
                        for(integer i=0; i<LINE_SIZE; i=i+1)  cache_mem[mem_rd_set_addr][mem_rd_way_addr][i] <= mem_rd_line[i];
