...
reg [19:0] led_clk_r;   //时钟分频、数码管刷新计数器
reg [4:0] led_sw_db_r;
reg [15:0] x_db_r;
reg xx_r, xx_1r;
wire x_p;
...
assign rst_cpu = rst;
assign clk_pdu = led_clk_r[1];      //PDU工作时钟25MHz
assign clk_db = led_clk_r[16];      //去抖动计数器时钟763Hz（周期约1.3ms）
...
cpu cpu (
  ...
  //Debug_BUS
  . pc(cpu_pc), 	        //当前执行指令地址
  .chk_addr(chk_addr),	    //数据通路状态的编码地址   最多256个数 2^16太多了
 .chk_data(cpu_chk_data)    //数据通路状态的数据
);
...
always @(posedge clk) begin
  if (rst) led_clk_r <= 20'h0;
  else led_clk_r <= led_clk_r + 20'h1;
end
...
always @(posedge clk_db) begin
  if (rst) led_sw_db_r <= 5'h0;
  else if (|(x ^ x_db_r))
    led_sw_db_r <= led_sw_db_r + 5'h1;
  else led_sw_db_r <= 5'h0;
end
...
always@(posedge clk_db) begin
  ...
  else if (led_sw_db_r[4]) begin    //信号稳定约21ms后输出
    x_db_r <= x;
    xx_r <= ~xx_r;
  end
end
...
///////////////////////////////////////////////
always @(posedge clk_db) begin
  if (rst) led_btn_db_r <= 5'h0;
  else if (|(btn ^ btn_db_r))
    led_btn_db_r <= led_btn_db_r + 5'h1;
  else led_btn_db_r <= 5'h0;
end

always@(posedge clk_db) begin
  if (rst) btn_db_r <= btn;
  else if (led_btn_db_r[4]) btn_db_r <= btn;
end
...
always@(posedge clk_cpu)begin
  if(rst) led_data_r <= 32'h0;
  else led_data_r <= led_data_r + 32'h1;
end
...
always @(*) begin          //数码管扫描
  case (led_clk_r[19:17])  //刷新频率约为95Hz
    3'b000: begin
      an_t <= 8'b1111_1110; 
