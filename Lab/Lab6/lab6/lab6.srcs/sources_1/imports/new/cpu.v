    ...
    case(chk_addr[13:12])
      2'b00:begin
        case(chk_addr[4:0])
            ...
            5'hF: chk_data = datamem_read_data_ext;
...
ME_reg ME_reg(
                ...
                .forward_data2(mux_data),//in forward_data2
...
WB_reg WB_reg(
                ...
                .dpra({14'b0,chk_addr[7:0],2'b0}),
                .dpo(m_data),
                .w_en(mem_write_ME),
                .load_data(datamem_read_data),
...
data_ext data_ext(//process the data one byte, half word, one word
            .data_in(datamem_read_data),
            .ld_bytes_select(ld_bytes_select),
            .reg_write_WB(reg_write_WB),
            .data_out(datamem_read_data_ext)
        );

assign reg_write_data = ~mem_to_reg_WB ? result_WB : datamem_read_data_ext;
